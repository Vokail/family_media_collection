...
export async function updateItem(id: string, patch: Partial<Pick<Item, 'is_wishlist' | 'notes' | 'rating' | 'cover_path' | 'status' | 'lego_status' | 'condition' | 'title' | 'creator' | 'year' | 'locked_fields'>>): Promise<Item> {
  const db = createServerClient()
